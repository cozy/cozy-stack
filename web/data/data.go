...
func defineIndex(c *gin.Context) {
	...
	result, err := couchdb.DefineIndexRaw(instance.GetDatabasePrefix(), doctype, &definitionRequest)
	if couchdb.IsNoDatabaseError(err) {
		if err = couchdb.CreateDB(instance.GetDatabasePrefix(), doctype); err == nil {
			result, err = couchdb.DefineIndexRaw(instance.GetDatabasePrefix(), doctype, &definitionRequest)
		}
	}
	if err != nil {
...
func findDocuments(c *gin.Context) {
	...
	err := couchdb.FindDocsRaw(instance.GetDatabasePrefix(), doctype, &findRequest, &results)
	if err != nil {
		c.AbortWithError(HTTPStatus(err), err)
		return
	}
