...
- [ ] **DEBT-01** : La race préexistante dans `pkg/config` / `model/stack` / `model/job.AntivirusTrigger` (FOLLOWUP-01 hérité de v1.1) est corrigée. `go test -race -count=1 ./web/webdav/...` s'exécute proprement (zéro WARNING: DATA RACE).
