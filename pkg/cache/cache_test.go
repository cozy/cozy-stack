...
func TestCache(t *testing.T) {
	...
	t.Run("MultiGet", func(t *testing.T) {
		...
		c := New(client)
		c.Set(ctx, "one", []byte("1"), 10*time.Millisecond)
		c.Set(ctx, "two", []byte("2"), 10*time.Millisecond)
		bufs := c.MultiGet(ctx, []string{"one", "two", "three"})
		assert.Len(t, bufs, 3)
	...
	t.Run("Keys", func(t *testing.T) {
		test := func(client redis.UniversalClient) {
			c := New(client)
			c.Set(ctx, "foo:one", []byte("1"), 10*time.Millisecond)
			c.Set(ctx, "foo:two", []byte("2"), 10*time.Millisecond)
			c.Set(ctx, "bar:baz", []byte("3"), 10*time.Millisecond)
			keys := c.Keys(ctx, "foo:")
			sort.Strings(keys)
