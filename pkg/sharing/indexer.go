...
// and to keep it for later. At least 3 revisions is needed for this to work
// (the first revision is the one that is already is CouchDB, the second is the
// revision for the next operation, and the third is the stash). If don't have
// them, we fallback to revisions generated by CouchDB.
func (s *sharingIndexer) StashRevision() string {
	if s.bulkRevs == nil || len(s.bulkRevs.Revisions.IDs) < 3 {
		s.bulkRevs = nil
		return ""
	}
