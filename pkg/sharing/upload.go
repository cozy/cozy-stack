...
//
// TODO if file was renamed + its content has changed, we should first modify
// the content, then rename it, not trying to do both at the same time. And we
// should do it in this order because the difficult case is if one operation
// succeeds and the other fails (if the two suceeds, we are fine; if the two
// fails, we just retry), and in that case, it is easier to manage a conflict
// on dir_id+name than on content: a conflict on different content should be
// resolved by a copy of the file, a conflict of name+dir_id, the higher
...
// revision after the renaming, it should be fine. Else, there is a risk that
...
func (s *Sharing) HandleFileUpload(inst *instance.Instance, key string, body io.ReadCloser) (err error) {
	...
	inst.Logger().WithField("nspace", "upload").
		Debugf("olddoc = %#v\n", olddoc)
	file, err := fs.CreateFile(target.FileDoc, olddoc)
	if err != nil {
		inst.Logger().WithField("nspace", "upload").
