...
func EncryptCredentialsData(data interface{}) ([]byte, error) {
	encryptorKey := config.GetVault().CredentialsEncryptorKey()
	if encryptorKey == nil {
		return nil, errCannotEncrypt
	}
	...
	return EncryptBufferWithKey(encryptorKey, buf)
}
...
// content.
func DecryptCredentialsData(encryptedData []byte) (interface{}, error) {
	decryptorKey := config.GetVault().CredentialsDecryptorKey()
	if decryptorKey == nil {
		return nil, errCannotDecrypt
	}
	plainBuffer, err := DecryptBufferWithKey(decryptorKey, encryptedData)
	if err != nil {
		return nil, err
	}
	var data interface{}
